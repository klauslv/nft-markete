...
contract('StarXERC1155Token', (accounts) => {
    ...
    describe('Test StarX1155 Token', async() => {
        ...
        it('7st test, transfer fail when transfer of token is not own', async() => {
            await expectRevert(starX1155Token.safeTransferFrom(user, buyer, 2, 11, '0x0', { from: owner }), 'caller is not owner nor approved')
        })
        ...
        it('10st test,set new baseURI', async() => {
            ...
            assert.equal(await starX1155Token.uri(curTokenId), newBaseURI + curTokenId.toString())
        })

        it('11st, mint batch ', async() => {
            const mintAmounts = [new BN(50), new BN(100), new BN(150)]
            await starX1155Token.mintBatch(test2, mintAmounts, data, { from: test2 });
            const tokenId = await starX1155Token.getCurTokenId()
            const balance1 = await starX1155Token.balanceOf(test2, tokenId)
            assert.equal(balance1.toString(), '150')
            const balance2 = await starX1155Token.balanceOf(test2, tokenId.sub(new BN(1)))
            assert.equal(balance2.toString(), '100')
            const balance3 = await starX1155Token.balanceOf(test2, tokenId.sub(new BN(2)))
        ...
        it('12st test, token transfer batch', async() => {
            ...
            const tokenId = await starX1155Token.getCurTokenId()
            const tokenIds = [tokenId.sub(new BN(3)), tokenId.sub(new BN(2)), tokenId.sub(new BN(1)), tokenId]
            ...
            assert.equal(totalSupply.toString(), tokenAmount[3].toString())
            expect(await starX1155Token.balanceOf(test2, tokenId)).to.be.bignumber.equal(tokenAmount[3])
        ...
        it('13st test, Indicates weither any token exist with a given id', async() => {
            await starX1155Token.mint(test2, '10', data, { from: test2 })
            const tokenId = await starX1155Token.getCurTokenId()
